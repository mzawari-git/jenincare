...
<section class="py-16 relative overflow-hidden" style="border-top:2px solid var(--glass-border);">
    <div class="max-w-4xl mx-auto px-4 text-center relative z-10">
        <h2 class="text-2xl md:text-3xl font-black mb-3" style="color:var(--brand-500);">????? ??? ????? {{ $siteSettings['site_name'] ?? '???? ???? ???????' }}</h2>
    ...
    <div class="max-w-7xl mx-auto px-4 relative z-10">
            ...
            <div class="lg:col-span-4 text-right">
                <div class="flex items-center gap-3 mb-5 justify-end">
                    @if(!empty($siteSettings['site_logo_url']))
                        <img src="{{ $siteSettings['site_logo_url'] }}" alt="{{ $siteSettings['site_name'] ?? '???? ???? ???????' }}" class="h-10 w-auto object-contain" style="max-height:40px;">
                    @else
                        <span class="text-2xl font-extrabold text-white">{{ $siteSettings['site_name'] ?? '???? ???? ???????' }}<span style="color:var(--brand-500);">.</span></span>
                    @endif
                </div>
                ...
                <div class="flex gap-3 justify-end">
                    @if(!empty($siteSettings['facebook_url']))<a href="{{ $siteSettings['facebook_url'] }}" target="_blank" class="w-9 h-9 rounded-full border flex items-center justify-center text-white/40 hover:text-white transition-all" style="border-color:var(--glass-border);" aria-label="??????"><i class="ph-fill ph-facebook-logo text-lg"></i></a>@endif
                    @if(!empty($siteSettings['instagram_url']))<a href="{{ $siteSettings['instagram_url'] }}" target="_blank" class="w-9 h-9 rounded-full border flex items-center justify-center text-white/40 hover:text-white transition-all" style="border-color:var(--glass-border);" aria-label="????????"><i class="ph-fill ph-instagram-logo text-lg"></i></a>@endif
                    ...
                    @if(!empty($siteSettings['tiktok_url']))<a href="{{ $siteSettings['tiktok_url'] }}" target="_blank" class="w-9 h-9 rounded-full border flex items-center justify-center text-white/40 hover:text-white transition-all" style="border-color:var(--glass-border);" aria-label="??? ???"><i class="ph-fill ph-tiktok-logo text-lg"></i></a>@endif
                </div>
            </div>
        ...
        <div class="pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-sm text-ink-dim" style="border-top:2px solid var(--glass-border);">
            ...
            <div class="flex gap-2 text-xs">
                ...
                @if(($siteSettings['payment_jawwal_enabled'] ?? '0') == '1')<span class="px-3 py-1 rounded-full" style="color:var(--brand-500);border:1px solid var(--glass-border);">???? ???</span>@endif
            </div>
        </div>
    </div>
</footer>
