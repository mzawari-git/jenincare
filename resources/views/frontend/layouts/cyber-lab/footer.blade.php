...
<section class="py-16 relative overflow-hidden" style="border-top:1px solid rgba(255,255,255,0.06);">
    ...
    <div class="max-w-4xl mx-auto px-4 text-center relative z-10">
        ...
        <form class="max-w-md mx-auto relative flex items-center" id="newsletterFormV3">
            @csrf
            <input type="email" name="email" placeholder="أدخلي بريدك الإلكتروني" required class="w-full bg-white/5 border border-white/10 text-white py-3.5 px-6 pl-28 rounded-full focus:outline-none focus:border-brand-500 transition-all text-sm placeholder:text-white/30">
    ...
    <div class="max-w-7xl mx-auto px-4">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div class="flex flex-col items-center text-center p-5 rounded-2xl border border-white/5 bg-white/[0.02]">
                ...
                <p class="text-ink-dim text-xs">ضمان الجودة والثقة</p>
            </div>
            <div class="flex flex-col items-center text-center p-5 rounded-2xl border border-white/5 bg-white/[0.02]">
                <div class="w-12 h-12 bg-brand-500/10 rounded-full flex items-center justify-center mb-3">
            ...
            <div class="flex flex-col items-center text-center p-5 rounded-2xl border border-white/5 bg-white/[0.02]">
                ...
                <p class="text-ink-dim text-xs">دعم فني على مدار الساعة 9 ص - 10 م</p>
            </div>
        </div>
    </div>
</section>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 mb-12">
            ...
            <div class="lg:col-span-4 text-right">
                ...
                <p class="text-ink-dim text-sm leading-relaxed mb-6 max-w-sm ml-auto">
                    {{ $siteSettings['site_description'] ?? 'منتجات تجميل أصلية للعناية بالبشرة والشعر مع خدمة توصيل سريعة وآمنة في فلسطين. شحن لكافة المدن الفلسطينية.' }}
                </p>
                <div class="flex gap-3 justify-end">
                    @if(!empty($siteSettings['instagram_url']))
                    <a href="{{ $siteSettings['instagram_url'] }}" target="_blank" class="w-9 h-9 rounded-full border border-white/10 flex items-center justify-center text-white/40 hover:bg-brand-500 hover:border-brand-500 hover:text-white transition-all" aria-label="انستغرام"><i class="ph-fill ph-instagram-logo text-lg"></i></a>
                    @endif
                    @if(!empty($siteSettings['facebook_url']))
                    <a href="{{ $siteSettings['facebook_url'] }}" target="_blank" class="w-9 h-9 rounded-full border border-white/10 flex items-center justify-center text-white/40 hover:bg-[#1877F2] hover:border-[#1877F2] hover:text-white transition-all" aria-label="واتساب"><i class="ph-fill ph-facebook-logo text-lg"></i></a>
                    @endif
...
document.getElementById('newsletterFormV3')?.addEventListener('submit', async function(e) {
    ...
    try {
        ...
        msgEl.innerHTML = '<span class="text-red-400">حدث خطأ من فضلك حاولي مرة أخرى</span>';
    }
    btn.disabled = false;
