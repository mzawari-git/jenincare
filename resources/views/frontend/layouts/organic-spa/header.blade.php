...
<header class="fixed top-0 w-full z-50 border-b-2" id="mainHeaderV3" style="background: rgba(12, 10, 12, 0.92) !important; backdrop-filter: blur(18px); -webkit-backdrop-filter: blur(18px); border-color: var(--glass-border); border-radius: 0 0 2rem 2rem; transition: background 0.3s, border-radius 0.3s;">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-14 md:h-16 flex items-center justify-between">
        <div class="flex items-center gap-8 flex-1">
            <a href="{{ route('home') }}" class="flex items-center gap-2 flex-shrink-0 group" style="color: var(--ink);">
                ...
                <div class="relative flex items-center justify-center rounded-lg p-1 transition-all duration-300 group-hover:scale-105" style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.05);">
                    <img src="{{ $siteSettings['site_logo_url'] }}" alt="{{ $siteSettings['site_name']??'???? ???? ???????' }}" class="h-6 md:h-10 w-auto object-contain" style="max-height:40px;max-width:120px;">
                </div>
                @else
                <span class="text-lg md:text-xl font-black tracking-tight">{{ $siteSettings['site_name_ar']??$siteSettings['site_name']??'???? ???? ???????' }}<span class="text-brand-500">.</span></span>
                @endif
            </a>
        ...
        <div class="flex items-center gap-3">
            <button onclick="toggleSearchV3()" class="icon-btn" aria-label="???"><i class="ph ph-magnifying-glass text-lg"></i></button>
            <a href="{{ route('cart') }}" class="icon-btn relative" aria-label="?????"><i class="ph ph-shopping-bag text-lg"></i><span class="absolute -top-0.5 -right-0.5 bg-brand-500 text-white text-[9px] font-bold h-4 w-4 rounded-full flex items-center justify-center" id="cart-count-v3">{{ $cartCount??0 }}</span></a>
            @auth<a href="{{ route('account') }}" class="hidden sm:flex items-center gap-1.5 text-sm font-medium nav-link"><i class="ph ph-user-circle"></i> ?????</a>@else<a href="{{ route('login') }}" class="hidden sm:inline-flex btn-ghost text-sm">????</a>@endauth
            <a href="{{ route('shop') }}" class="btn-primary text-sm inline-flex"><i class="ph ph-storefront"></i> <span class="hidden sm:inline">???? ????</span></a>
...
<div id="searchOverlayV3" class="fixed inset-0 z-[60] hidden items-start justify-center pt-32" style="background:rgba(0,0,0,0.7);backdrop-filter:blur(4px);">
    <div class="spa-card rounded-2xl w-full max-w-lg mx-4 p-6"><button onclick="toggleSearchV3()" class="absolute top-3 left-3 text-ink-dim text-xl">&times;</button><form action="{{ route('shop') }}" method="GET" class="flex gap-2"><input type="text" name="search" placeholder="????? ?? ????..." autofocus class="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-brand-500" style="color:var(--ink);"><button type="submit" class="btn-primary">???</button></form></div>
</div>
...
<div id="mobileMenuV3" class="fixed inset-0 z-[60] hidden" style="direction:rtl;">
    ...
    <div class="absolute top-0 right-0 w-[300px] sm:w-80 h-full shadow-2xl transform translate-x-full transition-transform duration-300 flex flex-col" id="mobileMenuPanelV3" style="background:var(--surface);">
        ...
        <div class="relative overflow-hidden px-5 py-5 border-b shrink-0" style="border-color:rgba(255,255,255,0.06);background:linear-gradient(135deg,var(--surface) 0%,rgba(var(--brand-500-rgb,255,42,133),0.08) 100%);">
            <div class="flex items-center justify-between relative z-10">
                <div class="flex items-center gap-2.5">
                    @if(!empty($siteSettings['site_logo_url']))
                        <img src="{{ $siteSettings['site_logo_url'] }}" alt="???? ???? ???????" class="h-8 w-auto object-contain" style="max-height:32px;max-width:120px;">
                    @else
                        <span class="text-lg font-black" style="color:var(--ink);">{{ $siteSettings['site_name']??'???? ???? ???????' }}</span>
                    @endif
                </div>
            ...
            <p class="text-[10px] mt-3 font-bold tracking-wider uppercase" style="color:var(--brand-500);">?????? ????? ? ???? ?? ??????</p>
        </div>
        ...
        <div class="flex-1 overflow-y-auto px-4 py-3 space-y-1 min-h-0">
            <p class="text-[10px] font-bold tracking-wider uppercase mb-2 mt-2 px-1" style="color:var(--ink-dim);">??????? ????????</p>
            ...
            <p class="text-[10px] font-bold tracking-wider uppercase mb-2 mt-4 px-1" style="color:var(--ink-dim);">????? ??? ?????</p>
            <div class="space-y-1">
            ...
            <div class="border-t mt-4 pt-4" style="border-color:rgba(255,255,255,0.06);">
                <p class="text-[10px] font-bold tracking-wider uppercase mb-2 px-1" style="color:var(--ink-dim);">?????</p>
                @auth
